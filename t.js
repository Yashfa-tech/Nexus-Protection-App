(function (global) {
  ...
  `;
          ...
          ${showDetail ? `
            <div class="T1_protection-added">
              <div class="T1_icon-container-message">
                ${svg("check").replace(/stroke="currentColor"/g, `stroke="${state.colors.svgColor || state.colors.textColor}"`)}
              </div>
            ...
            <div class="T1_protection-message">
              <div class="T1_icon-container-message">
                ${svg("shield").replace(/stroke="currentColor"/g, `stroke="${state.colors.svgColor || state.colors.textColor}"`)}
              </div>
    ...
    function bindButtons() {
      const buttons = da(".T1_protection-button", state.container);
    ...
    function destroy() {
      try {
        if (state.container) {
          state.container.innerHTML = "";
          if (state.container._templateInstance) {
            delete state.container._templateInstance;
          }
        }
        const styleElement = d("#T1_template-styles");
  ...
  if (typeof document !== 'undefined') {
    ...
    if (document.readyState === 'loading') {
      ...
      setTimeout(initializeTemplates, 0);
    }
  }
