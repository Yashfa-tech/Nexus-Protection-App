(function (global) {
    ...
    function renderT2() {
      ...
      state.container.innerHTML = `
        <div class="T1_template2-container">
          ...
          ` : ""}
          ${showCheckoutMsg ? `<div class="T1_checkout-button-message"></div>` : ""}
        </div>`;
    }
  ...
  if (typeof document !== 'undefined') {
    function initializeTemplates() {
      ...
      // Priority 2: Check for customization_Template_widget (CustomizationModel container)
      const customizationContainer = document.getElementById('customization_Template_widget');
      if (customizationContainer) {
        // Only initialize in customization container if it exists
        let templateContainer = customizationContainer.querySelector('#Template-customization-container');
        if (!templateContainer) {
          templateContainer = document.createElement('div');
          templateContainer.id = 'Template-customization-container';
          templateContainer.className = 'Template-wrapper';
          customizationContainer.appendChild(templateContainer);
        }
        global.initializeTemplate('Template-customization-container');
        return; // Exit early, don't create global container
      }

      // Priority 3: Check for existing template-container
      ...
      // Priority 4: Only create default container if we're NOT in a React app environment
      // Check if we're in a React environment by looking for React root
      const reactRoot = document.getElementById('root');
      if (reactRoot) {
        // We're in a React app, don't auto-initialize globally
        console.log('Template script loaded in React environment. Use ExternalTemplate component for controlled initialization.');
        return;
      }
